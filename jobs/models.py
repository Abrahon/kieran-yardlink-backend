...
#         # IN PROGRESS
#         # -------------------------
#         elif completed_items < total_items or not (has_before and has_after):
...
#     class Status(models.TextChoices):
#         UPCOMING = "upcoming", "Upcoming"        # after booking accepted
#         IN_PROGRESS = "in_progress", "In Progress"  # work started manually
#         COMPLETED = "completed", "Completed"
#         RESCHEDULED = "rescheduled", "Rescheduled"
#         CANCELLED = "cancelled", "Cancelled"
...
#     # ======================================================
#     # ✅ STATUS LOGIC (FIXED AS YOU WANT)
#     # ======================================================

#     # def sync_status(self, save=True):
#     #     """
#     #     RULES:
#     #     1. After booking → ALWAYS UPCOMING
#     #     2. Only manual action OR item progress → IN_PROGRESS
#     #     3. Only full completion + images → COMPLETED
#     #     """

#     #     total_items = self.items.count()
#     #     completed_items = self.items.filter(is_completed=True).count()

#     #     has_before = self.images.filter(image_type="before").exists()
#     #     has_after = self.images.filter(image_type="after").exists()
...
#     #     # -------------------------
#     #     # IN_PROGRESS (ONLY if real work started)
#     #     # -------------------------
#     #     elif (
#     #         completed_items > 0
#     #         or has_before
...
#     #     # -------------------------
#     #     # UPCOMING (DEFAULT AFTER BOOKING)
#     #     # -------------------------
#     #     else:
#     #         new_status = self.Status.UPCOMING
...
#         has_before = self.images.filter(image_type=JobImage.ImageType.BEFORE).exists()
#         has_after = self.images.filter(image_type=JobImage.ImageType.AFTER).exists()

#         # 🟢 COMPLETED (ONLY after images exist)
#         if (
#             total_items > 0
#             and completed_items == total_items
#             and has_before
...
#         # 🔵 IN PROGRESS (work started OR images started)
#         elif (
#             completed_items > 0
#             or has_before
#             or has_after
...
#         # ✅ FIXED: sync job status properly
#         self.job.sync_status()
...
class JobItem(TimeStampedModel):
    ...
    completed_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="completed_job_items"
    )
    ...
    def clean(self):
        ...
        if (
            self.item_type == self.ItemType.STANDARD_SERVICE
            and not self.service
        ):
            raise ValidationError(
                "Standard service item must have a service."
            )
        ...
        if (
            ...
            raise ValidationError(
                "Addon item must have an addon."
            )

        if (
            self.item_type == self.ItemType.CUSTOM
            and not self.name
        ):
            raise ValidationError(
                "Custom item must have a name."
            )
